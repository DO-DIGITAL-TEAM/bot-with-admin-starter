...
export function LoginForm({ onSuccess, ...props }: LoginFormProps) {
  ...
  const handleSubmit = form.onSubmit((variables) => {
    login(
      { variables },
      {
        onSuccess: (data) => setIsAuthenticated(true),
  ...
  return (
    <FormProvider form={form} onSubmit={handleSubmit}>
      <Stack {...props}>
        <TextInput name="email" label="Email" placeholder="john.doe@example.com" required />
        <PasswordInput name="password" label="Password" placeholder="Your password" required />
